...
class Monster {
  ...
  collide(state) {
    ...
    if (
      player.speed.y > 0 &&
      Math.abs(player.pos.y + player.size.y - this.pos.y) < 0.5
    ) {
      let filtered = state.actors.filter((a) => a != this);
...
class CanvasDisplay {
  ...
  drawPlayer(player, x, y, width, height) {
    ...
    this.cx.save();
    if (this.flipPlayer) {
      flipHorizentally(this.cx, x + width / 2);
    }
...
function flipHorizentally(contex, around) {
  contex.translate(around, 0);
  contex.scale(-1, 1);
  contex.translate(-around, 0);
}
...
async function runGame(plans, Display) {
  let lives = 3;
  let livesnode = document.createElement("div");
